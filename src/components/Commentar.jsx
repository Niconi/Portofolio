...
const Comment = memo(({ comment, formatDate, index, isPinned = false }) => (
    <div 
        className={`px-4 pt-4 pb-2 rounded-xl border transition-all group hover:shadow-lg hover:-translate-y-0.5 ${
            isPinned 
                ? 'bg-gradient-to-r from-primary/10 to-secondary/10 border-primary/30 hover:bg-gradient-to-r hover:from-primary/15 hover:to-secondary/15' 
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2" data-aos="fade-up" data-aos-duration="1000">
                <label className="block text-sm font-medium text-white">
                    {text.Contact.nameLabel} <span className="text-red-400">*</span>
                </label>
                <input
                    type="text"
                    value={userName}
                    ...
                    className="w-full p-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder-gray-400 focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all"
                    required
                />
            </div>
            ...
            <div className="space-y-2" data-aos="fade-up" data-aos-duration="1200">
                <label className="block text-sm font-medium text-white">
                    {text.Contact.messageLabel} <span className="text-red-400">*</span>
                </label>
                <textarea
                    ref={textareaRef}
                    value={newComment}
                    ...
                    className="w-full p-4 rounded-xl bg-white/5 border border-white/10 text-white placeholder-gray-400 focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all resize-none min-h-[120px]"
                    required
                />
            </div>
            ...
            <div className="space-y-2" data-aos="fade-up" data-aos-duration="1400">
                <label className="block text-sm font-medium text-white">
                    {text.Contact.profilePhotoLabel} <span className="text-gray-400">{text.Contact.optional}</span>
                </label>
                <div className="flex items-center gap-4 p-4 bg-white/5 border border-white/10 rounded-xl">
                    {imagePreview ? (
                        <div className="flex items-center gap-4">
                            ...
                            >
                                <X className="w-4 h-4" />
                                <span>{text.Contact.removePhoto}</span>
                            </button>
                        </div>
                    ) : (
                        <div className="w-full">
                            ...
                            >
                                ...
                                <span>{text.Contact.choosePhoto}</span>
                            </button>
                            <p className="text-center text-gray-400 text-sm mt-2">
                                {text.Contact.maxSize}
                            </p>
                        </div>
                    )}
                </div>
                ...
                <div className="relative flex items-center justify-center gap-2">
                    {isSubmitting ? (
                        <>
                            ...
                            <span>{text.Contact.posting}</span>
                        </>
                    ) : (
                        <>
                            <Send className="w-4 h-4" />
                            <span>{text.Contact.postComment}</span>
                        </>
                    )}
                </div>
            </button>
            ...
            <div className="p-6 border-b border-white/10" data-aos="fade-down" data-aos-duration="800">
                <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-xl font-semibold text-white">
                        {text.Contact.comments} <span className="text-primary">({totalComments})</span>
                    </h3>
                </div>
            </div>
                ...
                <div className="space-y-4 h-[328px] overflow-y-auto overflow-x-hidden custom-scrollbar pt-1 pr-1 " data-aos="fade-up" data-aos-delay="200">
                    ...
                    {comments.length === 0 && !pinnedComment ? (
                        <div className="text-center py-8" data-aos="fade-in">
                            ...
                            <p className="text-gray-400">{text.Contact.noComments}</p>
                        </div>
                    ) : (
